...
namespace Schema.Unity.Editor
{
    public class AttributeSettingsPrompt : EditorWindow
    {
            ...
            using (var identifierCheck = new EditorGUI.ChangeCheckScope())
            {
                ...
                if (identifierCheck.changed)
                {
                    if (setIsIdentifier)
                    {
                        ...
                        else if (scheme.GetAttributeValues(attribute)
                                 .GroupBy(v => v)
                                 .Any(g => g.Count() > 1))
                        {
                    ...
                    {
                        // TODO: validate that no other schema is referencing this identifier
        ...
        private void SaveSettings()
        {
            ...
            Core.Schema.SaveSchema(scheme, saveManifest: false);
